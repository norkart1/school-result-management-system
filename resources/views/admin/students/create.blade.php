...
<body style="background-color: #f8f9fa; font-family: Arial, sans-serif;">
    ...
    <nav style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 1rem 0; margin-bottom: 2rem;">
        <div style="max-width: 1200px; margin: 0 auto; padding: 0 2rem; display: flex; justify-content: space-between; align-items: center;">
            <h1 style="color: white; margin: 0; font-size: 1.5rem;">Add New Student</h1>
            ...
            <form method="POST" action="{{ route('admin.students.store') }}">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                    <div>
                        ...
                        <input type="text" name="school_code" value="{{ old('school_code') }}" required 
                               style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;"
                ...
                <div style="margin-bottom: 1rem;">
                    ...
                    <select name="grade" required style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;">
                        ...
                        <option value="Top Plus" {{ old('grade') == 'Top Plus' ? 'selected' : '' }}>Top Plus</option>
                        <option value="Distinction" {{ old('grade') == 'Distinction' ? 'selected' : '' }}>Distinction</option>
                        <option value="First Class" {{ old('grade') == 'First Class' ? 'selected' : '' }}>First Class</option>
                        <option value="Second Class" {{ old('grade') == 'Second Class' ? 'selected' : '' }}>Second Class</option>
                        <option value="Third Class" {{ old('grade') == 'Third Class' ? 'selected' : '' }}>Third Class</option>
                        <option value="Failed" {{ old('grade') == 'Failed' ? 'selected' : '' }}>Failed</option>
                        <option value="Not Promoted" {{ old('grade') == 'Not Promoted' ? 'selected' : '' }}>Not Promoted</option>
                    </select>
                    @error('grade')
                        <div style="color: #e3342f; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</div>
                    @enderror
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <div id="subjects-container">
                        <div class="subject-row" style="display: grid; grid-template-columns: 2fr 1fr auto; gap: 0.5rem; margin-bottom: 0.5rem; align-items: center;">
                            <input type="text" name="subject_names[]" placeholder="Subject Name (e.g., الرياضيات)" 
